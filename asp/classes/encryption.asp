...
Function decrypt(byval sIn)
	...
	if instr(decrypt,"gbqmdfk")<>0 and instr(decrypt,"ghbnchom")<>0 then
		...
		'do nothing
		elseif aspl.isNumber(decrypt) then
		decrypt=cstr(round(aspl.convertnmbr(decrypt)/111,0))
	end if
...
Function EnCrypt(byval sIn)
	'if len(sIn)<6 then
	if aspl.isNumber(sIn) then
	sIn=aspl.convertnmbr(sIn)*111
	end if
	'end if
	sIn=aspl.convertStr(sIn)
	dim x, y, abfrom, abto
	EnCrypt="": ABFrom = ""
	For x = 0 To 25: ABFrom = ABFrom & Chr(65 + x): Next 
	For x = 0 To 25: ABFrom = ABFrom & Chr(97 + x): Next 
